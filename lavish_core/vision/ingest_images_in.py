"""
Lavish 'Wall Street Edition' — Image Ingestion → OCR → Parsed Labels
--------------------------------------------------------------------
- Scans data/images_in (recursive)
- OCR with PaddleOCR (if installed) → fallback to Tesseract
...
def _get_parser():
    ...
    logger.warning("⚠️ Using safe regex parser (parser_triggers/parser missing).")
...
def load_existing_jsonl(path: str) -> Dict[str, Dict[str, Any]]:
    """Return map cache_key → record (for dedupe/resume)."""
...
# ----------------------
def main():
    ap = argparse.ArgumentParser(description="Lavish Image Ingestion → OCR → Labels")
    ...
    logger.info(f"🖼  Files to process: {len(imgs)}")
    t0 = time.time()
    ...
    logger.info(f"Saved JSONL → {JSONL_PATH}  (+{len(new_records)} records)")
    ...
    if args.write_csv:
        ...
        logger.info(f"Saved CSV → {CSV_PATH}  (rows={len(all_records)})")
